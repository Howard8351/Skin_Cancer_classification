...
class skin_cancer_classification_model:
    ...
    def creat_data_list(self, training):
        ...
        class_weight_rate = [10., 1., 5., 10., 1., 10., 10.]
        class_weight = dict()
    ...
    def creat_model(self):
        ...
        #                                                     input_shape = self.input_image_size + (self.channels,),
        #                                                     pooling = 'avg')
        #pretraining_model = tf.keras.applications.InceptionResNetV2(include_top = False,
        #                                                     input_shape = self.input_image_size + (self.channels,),
        #                                                     pooling = 'avg')

        pretraining_model = tf.keras.applications.EfficientNetB4(include_top = False,
                                                             input_shape = self.input_image_size + (self.channels,),
                                                             pooling = 'avg')

        input_layer  = Input(shape = self.input_image_size + (self.channels,))
        model_output = pretraining_model(input_layer)
        model_output.trainable = False
        output_layer = layers.Dense(self.number_of_class, activation = 'softmax')(model_output)
    ...
    def train_loop(self):
        ...
        #step = 0
        #label_count = [0] * self.number_of_class
        #while step < self.train_step_each_epoch:
        #    iter_output = train_iter.get_next()
        #    label = iter_output[1]
        #    label = label * range(self.number_of_class)
        #    label = tf.math.reduce_sum(label, 1)
        #    label = tf.cast(label, tf.int32)
        #    for index in range(self.batch_size):
        #        label_count[label[index].numpy()] += 1
        #    step += 1
        ...
        #self.model.evaluate(x = self.train_dataset, steps = self.train_step_each_epoch)
    ...
    def load_model_and_training(self):
        ...
        callback_path              = "model_callback_output"
        tensorboard_path           = "tensorboard_output" 
        self.model = tf.keras.models.load_model(callback_path)
        self.model.summary()
